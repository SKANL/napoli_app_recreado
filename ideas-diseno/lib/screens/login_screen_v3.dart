...
class _LoginScreenV3State extends State<LoginScreenV3> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Container(
        // FONDO CON PATRÓN TIPO MANTEL (Rojo y Blanco - Tema Pizzería)
        decoration: BoxDecoration(
          color: const Color(0xFFFFFFFF),
        ),
        child: Stack(
          children: [
            // PATRÓN DE MANTEL - RAYAS DIAGONALES ROJO Y BLANCO
            Positioned.fill(
              child: CustomPaint(
                painter: TableclothPattern(),
              ),
            ),
...
/// CustomPainter para crear un patrón tipo mantel de pizzería
class TableclothPattern extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    final redPaint = Paint()..color = const Color.fromARGB(130, 217, 49, 37);
    final whitePaint = Paint()..color = const Color(0xFFFFFFFF);

    const stripeWidth = 30.0;

    // Llenar todo con blanco primero
    canvas.drawRect(
      Rect.fromLTWH(0, 0, size.width, size.height),
      whitePaint,
    );

    // Dibujar rayas diagonales de arriba-izquierda a abajo-derecha (/)
    for (double i = -size.height;
        i < size.width + size.height;
        i += stripeWidth * 2) {
      final path1 = Path();
      path1.moveTo(i, 0);
      path1.lineTo(i + stripeWidth, 0);
      path1.lineTo(i + stripeWidth + size.height, size.height);
      path1.lineTo(i + size.height, size.height);
      path1.close();
      canvas.drawPath(path1, redPaint);
    }

    // Dibujar rayas diagonales de arriba-derecha a abajo-izquierda (\)
    for (double i = -size.height;
        i < size.width + size.height;
        i += stripeWidth * 2) {
      final path2 = Path();
      path2.moveTo(size.width - i, 0);
      path2.lineTo(size.width - i - stripeWidth, 0);
      path2.lineTo(size.width - i - stripeWidth - size.height, size.height);
      path2.lineTo(size.width - i - size.height, size.height);
      path2.close();
  ...
  @override
  bool shouldRepaint(TableclothPattern oldDelegate) => false;
}
